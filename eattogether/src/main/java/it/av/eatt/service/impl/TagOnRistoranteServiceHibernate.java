...
public class TagOnRistoranteServiceHibernate extends ApplicationServiceHibernate<TagOnRistorante> implements TagOnRistoranteService {
     ...
     * (non-Javadoc)
     * 
     * @see it.av.eatt.service.TagOnRistoranteService#getAllByRistorante(it.av.eatt.ocm.model.Ristorante)
     */
    @Override
    public List<TagOnRistorante> getAllByRistorante(Ristorante risto) throws JackWicketException {
        Criterion crit = Restrictions.eq(TagOnRistorante.RISTORANTE, risto);
        List<TagOnRistorante> results = findByCriteria(crit);
        return results;
    }
    ...
    public TagOnRistorante insert(Tag tag, Ristorante ristorante) throws JackWicketException {
        return super.save(new TagOnRistorante(tag, ristorante));
    }
     ...
     */
    @Override
    public TagOnRistorante insert(String tag, Ristorante ristorante) throws JackWicketException {
        return super.save(new TagOnRistorante(new Tag(tag), ristorante));
    }
    ...
    public List<TagOnRistorante> find(String tag, Ristorante risto) throws JackWicketException {
        ...
        if (tagItem != null) {
            Criterion critOnTag = Restrictions.eq(TagOnRistorante.TAG, tagItem);
            Criterion critOnRisto = Restrictions.eq(TagOnRistorante.RISTORANTE, risto);
            return findByCriteria(critOnTag, critOnRisto);
        } else {
